...
function ParentWrapper(template, expression) {
  ...
  // Passing in the optional expression argument adds support for
  // ParentWrapper::resolve. An expression should be provided only if
  // this.expression.get(context) always returns the same value as
  // this.template.get(context, true). To ensure proper escaping, rendering
  // methods (get, getFragment, appendTo, and attachTo) should always use
  // this.template and ignore this.expression.
...
ParentWrapper.prototype.get = function(context, unescaped) {
  return this.template.get(context.forViewParent(), unescaped);
};
