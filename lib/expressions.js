...
FnExpression.prototype.get = function(context) {
  ...
  return this._apply(getFn, context, controller);
};
FnExpression.prototype._lookup = function(controller) {
  return controller[this.name];
};
FnExpression.prototype._apply = function(getFn, context, thisArg) {
  if (!getFn) {
    throw new Error('Function not found for: ' + this.name);
  }
  if (!this.args) return getFn.call(thisArg);
  var inputs = [];
  for (var i = 0, len = this.args.length; i < len; i++) {
    inputs.push(this.args[i].get(context));
  }
  var value = getFn.apply(thisArg, inputs);
...
OperatorExpression.prototype.get = function(context) {
  return this._apply(this.getFn, context);
};
